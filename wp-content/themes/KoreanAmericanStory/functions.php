...
add_filter( 'the_excerpt', 'do_shortcode', 'excerpt_text');
...
include_once('includes/skim-custom-meta.php');


include_once('includes/skim-functions.php');


include_once('includes/skim-theme-setting.php');
...
if(!function_exists('exerpt_text')) {

    function exerpt_text() {



	if(the_field('exerpt_text') != "") {	

		echo '<p class="post_excerpt">'.the_field('exerpt_text').'</p>';
...
add_image_size( 'blog-type-3', 490, 490, true );
...
function delay_class($classes) {
	...
	if($qode_options_infographer['delay'] == "yes") :
